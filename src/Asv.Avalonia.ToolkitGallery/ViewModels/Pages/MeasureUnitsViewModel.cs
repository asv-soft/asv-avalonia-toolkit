    ...
    public MeasureUnitsViewModel()
    {
        ...
        Observable
            ...
            .Subscribe(_ =>
            {
                ...
                if (_ % 3 == 0)
                {
                    TopStatus = (IndicatorStatusEnum)
                        Random.Shared.Next(0, Enum.GetNames(typeof(IndicatorStatusEnum)).Length);
                }
                ...
                while (isProgressRun)
                {
                    ...
                    for (var i = 0; i < 100; i++)
                    {
                        ...
                        RightTopStatus = (IndicatorStatusEnum)
                            Random.Shared.Next(
                                0,
                                Enum.GetNames(typeof(IndicatorStatusEnum)).Length
                            );
                        LeftTopStatus = (IndicatorStatusEnum)
                            Random.Shared.Next(
                                0,
                                Enum.GetNames(typeof(IndicatorStatusEnum)).Length
                            );
